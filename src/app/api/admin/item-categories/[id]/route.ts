...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (error || !category) return NextResponse.json({ error: '道具分类不存在' }, { status: 404 })
    return NextResponse.json(category)
  } catch { return NextResponse.json({ error: '获取道具分类失败' }, { status: 500 }) }
}
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(category)
  } catch { return NextResponse.json({ error: '更新道具分类失败' }, { status: 500 }) }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  } catch { return NextResponse.json({ error: '删除道具分类失败' }, { status: 500 }) }
}
