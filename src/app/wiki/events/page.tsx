...
export default function EventsWikiPage() {
  ...
  useEffect(() => {
    fetch('/api/wiki/events/categories')
      .then(res => res.json())
      .then(data => {
    ...
    <div className="min-h-screen bg-wiki-bg">
      ...
      <main className="container mx-auto px-4 py-6 md:py-8">
        <div className="text-sm text-wiki-text-muted mb-4 md:mb-6">
          ...
          <span className="text-wiki-text">活動一覽</span>
        </div>
        ...
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <div>
            ...
            <p className="text-wiki-text-muted text-sm mt-2">選擇活動分類，查看活動詳情</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={`/wiki/events/${category.slug}`}
