...
export default function EquipmentWikiPage() {
  ...
  useEffect(() => {
    fetch('/api/wiki/equipment/categories')
      .then(res => res.json())
      .then(data => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="container mx-auto px-4 py-6 md:py-8">
        <div className="text-sm text-gray-900-muted mb-4 md:mb-6">
          ...
          <span className="mx-2">/</span>
          <span className="text-gray-900">装备图鉴</span>
        </div>
        ...
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <div>
            ...
            <p className="text-gray-900-muted text-sm mt-2">选择装备分类，查看详细信息</p>
          </div>
          {isAdmin && (
            <Link
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={`/wiki/equipment/${category.slug}`}
