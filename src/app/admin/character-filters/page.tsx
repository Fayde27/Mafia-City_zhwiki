...
export default function AdminCharacterFiltersPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await fetch('/api/admin/character-filters')
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/character-filters/${id}`, { method: 'DELETE' })
  ...
  const handleSortChange = async (id: string, sortOrder: number) => {
    try {
      await fetch(`/api/admin/character-filters/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="card-hard rounded-lg p-6 mb-6">
          ...
          <div className="flex gap-4">
            ...
            >
              ...
              <option value="weapon">适配兵种</option>
            </select>
            <input
              type="text"
              value={newValue}
