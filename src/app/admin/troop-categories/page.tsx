...
export default function AdminTroopCategoryPage() {
  const router = useRouter()
  const { isAdmin, isLoaded } = useAdminAuth()
  const [categories, setCategories] = useState<TroopCategory[]>([])
  const [loading, setLoading] = useState(true)
  const [showModal, setShowModal] = useState(false)
  const [editingCategory, setEditingCategory] = useState<TroopCategory | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory
        ? `/api/admin/troop-categories/${editingCategory.id}`
        : '/api/admin/troop-categories'
  ...
  const handleEdit = (category: TroopCategory) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/admin/troop-categories/${id}`, { method: 'DELETE' })
      fetchCategories()
    } catch (err) {
      alert('刪除失敗')
    }
  ...
  const handleMoveUp = async (category: TroopCategory, index: number) => {
    if (index === 0) return
    ...
    try {
      await Promise.all([
        fetch(`/api/admin/troop-categories/${category.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...category, sortOrder: newSortOrder }),
        }),
        fetch(`/api/admin/troop-categories/${categories[index - 1].id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleMoveDown = async (category: TroopCategory, index: number) => {
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-wiki-text-muted text-sm mt-1">管理兵種圖鑑分類，新增、編輯或刪除分類</p>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-wiki-gray">
                <tr>
                  ...
                  <th className="text-left px-6 py-4 text-wiki-accent font-bold uppercase tracking-wider text-sm">兵種數</th>
              ...
              <tbody>
                {categories.map((category) => (
                  <tr key={category.id} className="border-t border-wiki-border hover:bg-wiki-gray/50">
                    ...
                    <td className="px-6 py-4 text-wiki-accent font-bold">{category._count.troops}</td>
