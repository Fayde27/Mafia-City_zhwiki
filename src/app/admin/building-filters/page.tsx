...
export default function AdminBuildingFiltersPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/building-filters/${id}`, { method: 'DELETE' })
  ...
  const handleSortChange = async (id: string, sortOrder: number) => {
    try {
      await fetch(`/api/admin/building-filters/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-heading font-bold text-[#e8c547] heading-hard">建筑筛选选项管理</h1>
            <p className="text-gray-900-muted text-sm mt-1">管理建筑图鉴筛选菜单的选项</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="card-hard rounded-lg p-6">
              <h3 className="text-lg font-bold text-[#e8c547] mb-4">建筑类型</h3>
