...
export default function AdminEquipmentFiltersPage() {
  ...
  const fetchOptions = async () => {
    try {
      const res = await fetch('/api/admin/equipment-filters')
  ...
  const handleSortChange = async (id: string, sortOrder: number) => {
    try {
      await fetch(`/api/admin/equipment-filters/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-wiki-text-muted text-sm mt-1">管理装备图鉴筛选菜单的选项</p>
          </div>
        ...
        <div className="card-hard rounded-lg p-6 mb-6">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="btn-hard text-white disabled:opacity-50 px-6"
            >
              添加
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
                  ...
                  {rarityOptions.map((opt) => (
                    <div key={opt.id} className="flex items-center gap-4 bg-wiki-gray rounded-lg p-3">
                      <input
                        ...
                        className="w-16 bg-wiki-darker border border-wiki-border px-2 py-1 text-wiki-text text-center"
                      />
            ...
            <div className="card-hard rounded-lg p-6">
              <h3 className="text-lg font-bold text-wiki-accent mb-4">装备类型</h3>
