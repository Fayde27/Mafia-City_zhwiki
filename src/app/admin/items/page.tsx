...
export default function AdminItemsPage() {
  ...
  const handleFilterAdd = async () => {
    ...
    try {
      const res = await fetch('/api/admin/item-filters', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: newType.trim(), value: newValue.trim(), categoryId: selectedCatId }) })
                ...
                <div className="bg-wiki-gray-light border border-wiki-border rounded-lg p-6 mb-6">
                  ...
                  <div className="flex gap-4 items-end">
                    <div className="flex-shrink-0">
                      ...
                      <input type="text" value={newType} onChange={ev => setNewType(ev.target.value)} list="types-items" className="w-40 bg-wiki-gray border-2 border-wiki-border px-3 py-2 text-wiki-text focus:border-wiki-accent focus:outline-none" placeholder="如：稀有度" />
                      <datalist id="types-items">{allTypes.map(t => <option key={t} value={t} />)}</datalist>
                    </div>
