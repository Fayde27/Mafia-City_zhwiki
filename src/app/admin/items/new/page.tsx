...
export default function AdminItemNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.categoryId) { alert('請選擇所屬分類'); return }
    setSaving(true)
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/admin/items/edit/${data.id}`)
      } else {
        ...
        <div className="flex gap-8">
          ...
          <div className="w-48 flex-shrink-0 hidden lg:block">
            <div className="sticky top-8 space-y-1">
              <div className="text-wiki-text-muted text-xs font-bold uppercase tracking-wider mb-3 px-3">新增道具</div>
              {SECTIONS.map(sec => (
          ...
          <div className="flex-1 space-y-8 min-w-0">
            ...
            <div className="flex items-center justify-between lg:hidden">
              <h1 className="text-xl font-bold text-wiki-text"><span className="text-wiki-accent mr-2">◆</span>新增道具</h1>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input value={form.name} onChange={e => set('name', e.target.value)}
                    className={inputCls} placeholder="如：建造加速（1分鐘）" />
                </div>
                <div>
                  <label className={labelCls}>URL Slug *</label>
                  <input value={form.slug} onChange={e => set('slug', e.target.value)}
                    className={inputCls} placeholder="英文小寫，如：speed-up-building-1m" />
                </div>
                <div>
