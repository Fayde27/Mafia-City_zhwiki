...
export default function AdminBuildingCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory
        ? `/api/admin/building-categories/${editingCategory.id}`
        : '/api/admin/building-categories'
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/admin/building-categories/${id}`, { method: 'DELETE' })
      fetchCategories()
    } catch (err) {
      alert('删除失败')
    }
  ...
  const handleMoveUp = async (category: BuildingCategory, index: number) => {
    if (index === 0) return
    ...
    try {
      await Promise.all([
        fetch(`/api/admin/building-categories/${category.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...category, sortOrder: newSortOrder }),
        }),
        fetch(`/api/admin/building-categories/${categories[index - 1].id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleMoveDown = async (category: BuildingCategory, index: number) => {
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-wiki-text-muted text-sm mt-1">管理建筑图鉴分类，新增、编辑或删除分类</p>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-wiki-gray">
                <tr>
                  ...
                  <th className="text-left px-6 py-4 text-wiki-accent font-bold uppercase tracking-wider text-sm">建筑数</th>
              ...
              <tbody>
                {categories.map((category) => (
                  <tr key={category.id} className="border-t border-wiki-border hover:bg-wiki-gray/50">
                    ...
                    <td className="px-6 py-4 text-wiki-accent font-bold">{category._count.buildings}</td>
