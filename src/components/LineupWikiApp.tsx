                      ...
                      return (
                        <div key={s.role} className="border-r border-wiki-border last:border-r-0">
                          ...
                          <div className="relative aspect-[3/4] bg-wiki-gray overflow-hidden">
                            {hero?.imgUrl ? <img src={hero.imgUrl} alt={hero.name} loading="lazy" decoding="async" className="w-full h-full object-cover" style={{ objectPosition: 'top center' }} /> : <div className="w-full h-full flex items-center justify-center text-wiki-text-muted text-[10px] sm:text-xs">{hero?.name || '未設置'}</div>}
                          ...
                          {kind === 'haojie' ? (
                            <>
                              {/* 武器：圖標+名稱一行，詞條另起並佔滿整欄寬（欄位窄，擠在圖標右側只剩 ~70px） */}
                              {weapon && (
                              ...
                              {emblem && (
                                <div className="px-1.5 sm:px-3 py-1.5 sm:py-2 border-t border-wiki-border">
                                  ...
                                  {(s.emblemAttrIds || []).map(id => attrMap[id]?.name).filter(Boolean).length > 0 && (
                                    <div className="cjk-text text-[10px] sm:text-[11px] text-wiki-text-muted leading-tight mt-1">
                              ...
                              {setMap[s.setId || ''] && (
                                <div className="px-1.5 sm:px-3 py-1.5 sm:py-2 border-t border-wiki-border">
                                  <div className="flex items-center gap-2">
                                    {setMap[s.setId!].imgUrl && <img src={setMap[s.setId!].imgUrl} loading="lazy" decoding="async" className="w-6 h-6 object-contain" alt="" />}
                                  ...
                                  {parseArr<string>(setMap[s.setId!].bonus).map((b, i) => (
                                    <div key={i} className="cjk-text text-[10px] sm:text-[11px] text-wiki-accent/90 leading-tight">✨ {b}</div>
                                  ))}
                                </div>
                              )}
                              {/* 6 格裝備 */}
